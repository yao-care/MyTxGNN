---
layout: drug
title: "Folic Acid"
drugbank_id: "DB00158"
drug_name: "Folic Acid"
kg_predictions: 132
dl_predictions: 50
evidence_level: "L1"
indication_count: 132
brands: []
permalink: /drugs/folic-acid/
---

# Folic Acid

## Quick Overview

| Attribute | Value |
|-----------|-------|
| **Drug Name** | Folic Acid |
| **DrugBank ID** | [DB00158](https://go.drugbank.com/drugs/DB00158) |
| **KG Predictions** | 132 |
| **DL Predictions (≥0.7)** | 50 |
| **Evidence Level** | L1 |
| **NPRA Status** | Approved |

---

## Sample Brand Names in Malaysia



---

## Predicted Indications

This drug has **132** knowledge graph predictions and **50** high-confidence deep learning predictions.

### Top KG Predictions

| Indication | Source |
|------------|--------|
| microcytic anemia | KG |
| vitamin deficiency disorder | KG |
| deficiency anemia | KG |
| folic acid deficiency anemia | KG |
| iron deficiency anemia | KG |
| hypochromic anemia | KG |
| microcytic anemia | KG |
| vitamin deficiency disorder | KG |
| deficiency anemia | KG |
| folic acid deficiency anemia | KG |

---

## FHIR Resources

Access drug data via FHIR R4 API:

- **MedicationKnowledge**: [`/fhir/MedicationKnowledge/DB00158`](/fhir/MedicationKnowledge/DB00158.json)

---

## Related Resources

| Resource | Link |
|----------|------|
| DrugBank | [View on DrugBank](https://go.drugbank.com/drugs/DB00158) |
| PubMed | [Search PubMed](https://pubmed.ncbi.nlm.nih.gov/?term=Folic+Acid) |
| ClinicalTrials.gov | [Search Trials](https://clinicaltrials.gov/search?intr=Folic+Acid) |

---

<div class="disclaimer">
<strong>Disclaimer</strong><br>
Drug repurposing predictions are for research purposes only. These predictions have not been clinically validated and should not be used for medical decisions. Always consult healthcare professionals.
</div>
