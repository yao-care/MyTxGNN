---
layout: drug
title: "Trastuzumab Emtansine"
drugbank_id: "DB05773"
drug_name: "Trastuzumab Emtansine"
kg_predictions: 2
dl_predictions: 0
evidence_level: "L5"
indication_count: 2
brands: []
permalink: /drugs/trastuzumab-emtansine/
---

# Trastuzumab Emtansine

## Gambaran Ringkas

| Atribut | Nilai |
|-----------|-------|
| **Nama Ubat** | Trastuzumab Emtansine |
| **ID DrugBank** | [DB05773](https://go.drugbank.com/drugs/DB05773) |
| **Ramalan KG** | 2 |
| **Ramalan DL (≥0.7)** | 0 |
| **Tahap Bukti** | L5 |
| **Status NPRA** | Approved |

---

## Contoh Nama Jenama di Malaysia



---

## Petunjuk Diramalkan

Ubat ini mempunyai **2** ramalan graf pengetahuan dan **0** ramalan pembelajaran mendalam berkeyakinan tinggi.

### Ramalan KG Teratas

| Petunjuk | Sumber |
|------------|--------|
| HER2 positive breast carcinoma | KG |
| HER2 positive breast carcinoma | KG |

---

## Sumber FHIR

Akses data ubat melalui API FHIR R4:

- **MedicationKnowledge**: [`/fhir/MedicationKnowledge/DB05773`](/fhir/MedicationKnowledge/DB05773.json)

---

## Sumber Berkaitan

| Sumber | Pautan |
|----------|------|
| DrugBank | [View on DrugBank](https://go.drugbank.com/drugs/DB05773) |
| PubMed | [Search PubMed](https://pubmed.ncbi.nlm.nih.gov/?term=Trastuzumab+Emtansine) |
| ClinicalTrials.gov | [Search Trials](https://clinicaltrials.gov/search?intr=Trastuzumab+Emtansine) |

---

<div class="disclaimer">
<strong>Penafian</strong><br>
Ramalan penggunaan semula ubat adalah untuk tujuan penyelidikan sahaja. Ramalan ini belum disahkan secara klinikal dan tidak boleh digunakan untuk keputusan perubatan. Sentiasa dapatkan nasihat profesional kesihatan.
</div>
