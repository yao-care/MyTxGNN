---
layout: drug
title: "Propylthiouracil"
drugbank_id: "DB00550"
drug_name: "Propylthiouracil"
kg_predictions: 4
dl_predictions: 0
evidence_level: "L1"
indication_count: 3
brands: []
permalink: /drugs/propylthiouracil/
---

# Propylthiouracil

## Gambaran Ringkas

| Atribut | Nilai |
|-----------|-------|
| **Nama Ubat** | Propylthiouracil |
| **ID DrugBank** | [DB00550](https://go.drugbank.com/drugs/DB00550) |
| **Ramalan KG** | 4 |
| **Ramalan DL (≥0.7)** | 0 |
| **Tahap Bukti** | L5 |
| **Status NPRA** | Approved |

---

## Contoh Nama Jenama di Malaysia



---

## Petunjuk Diramalkan

Ubat ini mempunyai **4** ramalan graf pengetahuan dan **0** ramalan pembelajaran mendalam berkeyakinan tinggi.

### Ramalan KG Teratas

| Petunjuk | Sumber |
|------------|--------|
| Graves disease | KG |
| thyroid crisis (disease) | KG |
| hyperthyroidism | KG |
| thyrotoxicosis | KG |

---

## Sumber FHIR

Akses data ubat melalui API FHIR R4:

- **MedicationKnowledge**: [`/fhir/MedicationKnowledge/DB00550`](/fhir/MedicationKnowledge/DB00550.json)

---

## Sumber Berkaitan

| Sumber | Pautan |
|----------|------|
| DrugBank | [View on DrugBank](https://go.drugbank.com/drugs/DB00550) |
| PubMed | [Search PubMed](https://pubmed.ncbi.nlm.nih.gov/?term=Propylthiouracil) |
| ClinicalTrials.gov | [Search Trials](https://clinicaltrials.gov/search?intr=Propylthiouracil) |

---

<div class="disclaimer">
<strong>Penafian</strong><br>
Ramalan penggunaan semula ubat adalah untuk tujuan penyelidikan sahaja. Ramalan ini belum disahkan secara klinikal dan tidak boleh digunakan untuk keputusan perubatan. Sentiasa dapatkan nasihat profesional kesihatan.
</div>
