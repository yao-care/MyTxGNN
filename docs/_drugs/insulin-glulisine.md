---
layout: drug
title: "Insulin Glulisine"
drugbank_id: "DB01309"
drug_name: "Insulin Glulisine"
kg_predictions: 2
dl_predictions: 0
evidence_level: "L1"
indication_count: 10
brands: []
permalink: /drugs/insulin-glulisine/
---

# Insulin Glulisine

## Gambaran Ringkas

| Atribut | Nilai |
|-----------|-------|
| **Nama Ubat** | Insulin Glulisine |
| **ID DrugBank** | [DB01309](https://go.drugbank.com/drugs/DB01309) |
| **Ramalan KG** | 2 |
| **Ramalan DL (≥0.7)** | 0 |
| **Tahap Bukti** | L5 |
| **Status NPRA** | Approved |

---

## Contoh Nama Jenama di Malaysia



---

## Petunjuk Diramalkan

Ubat ini mempunyai **2** ramalan graf pengetahuan dan **0** ramalan pembelajaran mendalam berkeyakinan tinggi.

### Ramalan KG Teratas

| Petunjuk | Sumber |
|------------|--------|
| diabetic ketoacidosis | KG |
| diabetes mellitus (disease) | KG |

---

## Sumber FHIR

Akses data ubat melalui API FHIR R4:

- **MedicationKnowledge**: [`/fhir/MedicationKnowledge/DB01309`](/fhir/MedicationKnowledge/DB01309.json)

---

## Sumber Berkaitan

| Sumber | Pautan |
|----------|------|
| DrugBank | [View on DrugBank](https://go.drugbank.com/drugs/DB01309) |
| PubMed | [Search PubMed](https://pubmed.ncbi.nlm.nih.gov/?term=Insulin+Glulisine) |
| ClinicalTrials.gov | [Search Trials](https://clinicaltrials.gov/search?intr=Insulin+Glulisine) |

---

<div class="disclaimer">
<strong>Penafian</strong><br>
Ramalan penggunaan semula ubat adalah untuk tujuan penyelidikan sahaja. Ramalan ini belum disahkan secara klinikal dan tidak boleh digunakan untuk keputusan perubatan. Sentiasa dapatkan nasihat profesional kesihatan.
</div>
