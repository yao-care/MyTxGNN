---
layout: drug
title: "Lorlatinib"
drugbank_id: "DB12130"
drug_name: "Lorlatinib"
kg_predictions: 4
dl_predictions: 0
evidence_level: "L5"
indication_count: 4
brands: []
permalink: /drugs/lorlatinib/
---

# Lorlatinib

## Gambaran Ringkas

| Atribut | Nilai |
|-----------|-------|
| **Nama Ubat** | Lorlatinib |
| **ID DrugBank** | [DB12130](https://go.drugbank.com/drugs/DB12130) |
| **Ramalan KG** | 4 |
| **Ramalan DL (≥0.7)** | 0 |
| **Tahap Bukti** | L5 |
| **Status NPRA** | Approved |

---

## Contoh Nama Jenama di Malaysia



---

## Petunjuk Diramalkan

Ubat ini mempunyai **4** ramalan graf pengetahuan dan **0** ramalan pembelajaran mendalam berkeyakinan tinggi.

### Ramalan KG Teratas

| Petunjuk | Sumber |
|------------|--------|
| lung cancer | KG |
| non-small cell lung carcinoma (disease) | KG |
| lung cancer | KG |
| non-small cell lung carcinoma (disease) | KG |

---

## Sumber FHIR

Akses data ubat melalui API FHIR R4:

- **MedicationKnowledge**: [`/fhir/MedicationKnowledge/DB12130`](/fhir/MedicationKnowledge/DB12130.json)

---

## Sumber Berkaitan

| Sumber | Pautan |
|----------|------|
| DrugBank | [View on DrugBank](https://go.drugbank.com/drugs/DB12130) |
| PubMed | [Search PubMed](https://pubmed.ncbi.nlm.nih.gov/?term=Lorlatinib) |
| ClinicalTrials.gov | [Search Trials](https://clinicaltrials.gov/search?intr=Lorlatinib) |

---

<div class="disclaimer">
<strong>Penafian</strong><br>
Ramalan penggunaan semula ubat adalah untuk tujuan penyelidikan sahaja. Ramalan ini belum disahkan secara klinikal dan tidak boleh digunakan untuk keputusan perubatan. Sentiasa dapatkan nasihat profesional kesihatan.
</div>
