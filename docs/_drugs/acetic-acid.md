---
layout: drug
title: "Acetic Acid"
drugbank_id: "DB03166"
drug_name: "Acetic Acid"
kg_predictions: 2
dl_predictions: 0
evidence_level: "L2"
indication_count: 9
brands: []
permalink: /drugs/acetic-acid/
---

# Acetic Acid

## Gambaran Ringkas

| Atribut | Nilai |
|-----------|-------|
| **Nama Ubat** | Acetic Acid |
| **ID DrugBank** | [DB03166](https://go.drugbank.com/drugs/DB03166) |
| **Ramalan KG** | 2 |
| **Ramalan DL (≥0.7)** | 0 |
| **Tahap Bukti** | L5 |
| **Status NPRA** | Approved |

---

## Contoh Nama Jenama di Malaysia



---

## Petunjuk Diramalkan

Ubat ini mempunyai **2** ramalan graf pengetahuan dan **0** ramalan pembelajaran mendalam berkeyakinan tinggi.

### Ramalan KG Teratas

| Petunjuk | Sumber |
|------------|--------|
| otitis externa | KG |
| external ear disease | KG |

---

## Sumber FHIR

Akses data ubat melalui API FHIR R4:

- **MedicationKnowledge**: [`/fhir/MedicationKnowledge/DB03166`](/fhir/MedicationKnowledge/DB03166.json)

---

## Sumber Berkaitan

| Sumber | Pautan |
|----------|------|
| DrugBank | [View on DrugBank](https://go.drugbank.com/drugs/DB03166) |
| PubMed | [Search PubMed](https://pubmed.ncbi.nlm.nih.gov/?term=Acetic+Acid) |
| ClinicalTrials.gov | [Search Trials](https://clinicaltrials.gov/search?intr=Acetic+Acid) |

---

<div class="disclaimer">
<strong>Penafian</strong><br>
Ramalan penggunaan semula ubat adalah untuk tujuan penyelidikan sahaja. Ramalan ini belum disahkan secara klinikal dan tidak boleh digunakan untuk keputusan perubatan. Sentiasa dapatkan nasihat profesional kesihatan.
</div>
