---
layout: drug
title: "Rimegepant"
drugbank_id: "DB12457"
drug_name: "Rimegepant"
kg_predictions: 2
dl_predictions: 0
evidence_level: "L2"
indication_count: 6
brands: []
permalink: /drugs/rimegepant/
---

# Rimegepant

## Gambaran Ringkas

| Atribut | Nilai |
|-----------|-------|
| **Nama Ubat** | Rimegepant |
| **ID DrugBank** | [DB12457](https://go.drugbank.com/drugs/DB12457) |
| **Ramalan KG** | 2 |
| **Ramalan DL (≥0.7)** | 0 |
| **Tahap Bukti** | L5 |
| **Status NPRA** | Approved |

---

## Contoh Nama Jenama di Malaysia



---

## Petunjuk Diramalkan

Ubat ini mempunyai **2** ramalan graf pengetahuan dan **0** ramalan pembelajaran mendalam berkeyakinan tinggi.

### Ramalan KG Teratas

| Petunjuk | Sumber |
|------------|--------|
| migraine with or without aura, susceptibility to | KG |
| migraine disorder | KG |

---

## Sumber FHIR

Akses data ubat melalui API FHIR R4:

- **MedicationKnowledge**: [`/fhir/MedicationKnowledge/DB12457`](/fhir/MedicationKnowledge/DB12457.json)

---

## Sumber Berkaitan

| Sumber | Pautan |
|----------|------|
| DrugBank | [View on DrugBank](https://go.drugbank.com/drugs/DB12457) |
| PubMed | [Search PubMed](https://pubmed.ncbi.nlm.nih.gov/?term=Rimegepant) |
| ClinicalTrials.gov | [Search Trials](https://clinicaltrials.gov/search?intr=Rimegepant) |

---

<div class="disclaimer">
<strong>Penafian</strong><br>
Ramalan penggunaan semula ubat adalah untuk tujuan penyelidikan sahaja. Ramalan ini belum disahkan secara klinikal dan tidak boleh digunakan untuk keputusan perubatan. Sentiasa dapatkan nasihat profesional kesihatan.
</div>
