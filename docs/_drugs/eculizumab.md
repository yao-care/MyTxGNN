---
layout: drug
title: "Eculizumab"
drugbank_id: "DB01257"
drug_name: "Eculizumab"
kg_predictions: 2
dl_predictions: 0
evidence_level: "L4"
indication_count: 10
brands: []
permalink: /drugs/eculizumab/
---

# Eculizumab

## Gambaran Ringkas

| Atribut | Nilai |
|-----------|-------|
| **Nama Ubat** | Eculizumab |
| **ID DrugBank** | [DB01257](https://go.drugbank.com/drugs/DB01257) |
| **Ramalan KG** | 2 |
| **Ramalan DL (≥0.7)** | 0 |
| **Tahap Bukti** | L5 |
| **Status NPRA** | Approved |

---

## Contoh Nama Jenama di Malaysia



---

## Petunjuk Diramalkan

Ubat ini mempunyai **2** ramalan graf pengetahuan dan **0** ramalan pembelajaran mendalam berkeyakinan tinggi.

### Ramalan KG Teratas

| Petunjuk | Sumber |
|------------|--------|
| paroxysmal nocturnal hemoglobinuria | KG |
| neuromyelitis optica | KG |

---

## Sumber FHIR

Akses data ubat melalui API FHIR R4:

- **MedicationKnowledge**: [`/fhir/MedicationKnowledge/DB01257`](/fhir/MedicationKnowledge/DB01257.json)

---

## Sumber Berkaitan

| Sumber | Pautan |
|----------|------|
| DrugBank | [View on DrugBank](https://go.drugbank.com/drugs/DB01257) |
| PubMed | [Search PubMed](https://pubmed.ncbi.nlm.nih.gov/?term=Eculizumab) |
| ClinicalTrials.gov | [Search Trials](https://clinicaltrials.gov/search?intr=Eculizumab) |

---

<div class="disclaimer">
<strong>Penafian</strong><br>
Ramalan penggunaan semula ubat adalah untuk tujuan penyelidikan sahaja. Ramalan ini belum disahkan secara klinikal dan tidak boleh digunakan untuk keputusan perubatan. Sentiasa dapatkan nasihat profesional kesihatan.
</div>
