---
layout: drug
title: "Lamivudine"
drugbank_id: "DB00709"
drug_name: "Lamivudine"
kg_predictions: 90
dl_predictions: 57
evidence_level: "L5"
indication_count: 90
brands: []
permalink: /drugs/lamivudine/
---

# Lamivudine

## Quick Overview

| Attribute | Value |
|-----------|-------|
| **Drug Name** | Lamivudine |
| **DrugBank ID** | [DB00709](https://go.drugbank.com/drugs/DB00709) |
| **KG Predictions** | 90 |
| **DL Predictions (≥0.7)** | 57 |
| **Evidence Level** | L5 |
| **NPRA Status** | Approved |

---

## Sample Brand Names in Malaysia



---

## Predicted Indications

This drug has **90** knowledge graph predictions and **57** high-confidence deep learning predictions.

### Top KG Predictions

| Indication | Source |
|------------|--------|
| HIV infectious disease | KG |
| chronic hepatitis B virus infection | KG |
| AIDS | KG |
| HIV infectious disease | KG |
| chronic hepatitis B virus infection | KG |
| AIDS | KG |
| HIV infectious disease | KG |
| chronic hepatitis B virus infection | KG |
| AIDS | KG |
| HIV infectious disease | KG |

---

## FHIR Resources

Access drug data via FHIR R4 API:

- **MedicationKnowledge**: [`/fhir/MedicationKnowledge/DB00709`](/fhir/MedicationKnowledge/DB00709.json)

---

## Related Resources

| Resource | Link |
|----------|------|
| DrugBank | [View on DrugBank](https://go.drugbank.com/drugs/DB00709) |
| PubMed | [Search PubMed](https://pubmed.ncbi.nlm.nih.gov/?term=Lamivudine) |
| ClinicalTrials.gov | [Search Trials](https://clinicaltrials.gov/search?intr=Lamivudine) |

---

<div class="disclaimer">
<strong>Disclaimer</strong><br>
Drug repurposing predictions are for research purposes only. These predictions have not been clinically validated and should not be used for medical decisions. Always consult healthcare professionals.
</div>
