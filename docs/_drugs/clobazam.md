---
layout: drug
title: "Clobazam"
drugbank_id: "DB00349"
drug_name: "Clobazam"
kg_predictions: 2
dl_predictions: 0
evidence_level: "L5"
indication_count: 10
brands: []
permalink: /drugs/clobazam/
---

# Clobazam

## Gambaran Ringkas

| Atribut | Nilai |
|-----------|-------|
| **Nama Ubat** | Clobazam |
| **ID DrugBank** | [DB00349](https://go.drugbank.com/drugs/DB00349) |
| **Ramalan KG** | 2 |
| **Ramalan DL (≥0.7)** | 0 |
| **Tahap Bukti** | L5 |
| **Status NPRA** | Approved |

---

## Contoh Nama Jenama di Malaysia



---

## Petunjuk Diramalkan

Ubat ini mempunyai **2** ramalan graf pengetahuan dan **0** ramalan pembelajaran mendalam berkeyakinan tinggi.

### Ramalan KG Teratas

| Petunjuk | Sumber |
|------------|--------|
| cutis verticis gyrata | KG |
| Lennox-Gastaut syndrome | KG |

---

## Sumber FHIR

Akses data ubat melalui API FHIR R4:

- **MedicationKnowledge**: [`/fhir/MedicationKnowledge/DB00349`](/fhir/MedicationKnowledge/DB00349.json)

---

## Sumber Berkaitan

| Sumber | Pautan |
|----------|------|
| DrugBank | [View on DrugBank](https://go.drugbank.com/drugs/DB00349) |
| PubMed | [Search PubMed](https://pubmed.ncbi.nlm.nih.gov/?term=Clobazam) |
| ClinicalTrials.gov | [Search Trials](https://clinicaltrials.gov/search?intr=Clobazam) |

---

<div class="disclaimer">
<strong>Penafian</strong><br>
Ramalan penggunaan semula ubat adalah untuk tujuan penyelidikan sahaja. Ramalan ini belum disahkan secara klinikal dan tidak boleh digunakan untuk keputusan perubatan. Sentiasa dapatkan nasihat profesional kesihatan.
</div>
