---
layout: drug
title: "Tropicamide"
drugbank_id: "DB00809"
drug_name: "Tropicamide"
kg_predictions: 1
dl_predictions: 0
evidence_level: "L5"
indication_count: 3
brands: []
permalink: /drugs/tropicamide/
---

# Tropicamide

## Gambaran Ringkas

| Atribut | Nilai |
|-----------|-------|
| **Nama Ubat** | Tropicamide |
| **ID DrugBank** | [DB00809](https://go.drugbank.com/drugs/DB00809) |
| **Ramalan KG** | 1 |
| **Ramalan DL (≥0.7)** | 0 |
| **Tahap Bukti** | L5 |
| **Status NPRA** | Approved |

---

## Contoh Nama Jenama di Malaysia



---

## Petunjuk Diramalkan

Ubat ini mempunyai **1** ramalan graf pengetahuan dan **0** ramalan pembelajaran mendalam berkeyakinan tinggi.

### Ramalan KG Teratas

| Petunjuk | Sumber |
|------------|--------|
| cycloplegia | KG |

---

## Sumber FHIR

Akses data ubat melalui API FHIR R4:

- **MedicationKnowledge**: [`/fhir/MedicationKnowledge/DB00809`](/fhir/MedicationKnowledge/DB00809.json)

---

## Sumber Berkaitan

| Sumber | Pautan |
|----------|------|
| DrugBank | [View on DrugBank](https://go.drugbank.com/drugs/DB00809) |
| PubMed | [Search PubMed](https://pubmed.ncbi.nlm.nih.gov/?term=Tropicamide) |
| ClinicalTrials.gov | [Search Trials](https://clinicaltrials.gov/search?intr=Tropicamide) |

---

<div class="disclaimer">
<strong>Penafian</strong><br>
Ramalan penggunaan semula ubat adalah untuk tujuan penyelidikan sahaja. Ramalan ini belum disahkan secara klinikal dan tidak boleh digunakan untuk keputusan perubatan. Sentiasa dapatkan nasihat profesional kesihatan.
</div>
