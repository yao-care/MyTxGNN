---
layout: drug
title: "Ethionamide"
drugbank_id: "DB00609"
drug_name: "Ethionamide"
kg_predictions: 1
dl_predictions: 0
evidence_level: "L5"
indication_count: 1
brands: []
permalink: /drugs/ethionamide/
---

# Ethionamide

## Gambaran Ringkas

| Atribut | Nilai |
|-----------|-------|
| **Nama Ubat** | Ethionamide |
| **ID DrugBank** | [DB00609](https://go.drugbank.com/drugs/DB00609) |
| **Ramalan KG** | 1 |
| **Ramalan DL (≥0.7)** | 0 |
| **Tahap Bukti** | L5 |
| **Status NPRA** | Approved |

---

## Contoh Nama Jenama di Malaysia



---

## Petunjuk Diramalkan

Ubat ini mempunyai **1** ramalan graf pengetahuan dan **0** ramalan pembelajaran mendalam berkeyakinan tinggi.

### Ramalan KG Teratas

| Petunjuk | Sumber |
|------------|--------|
| pulmonary tuberculosis | KG |

---

## Sumber FHIR

Akses data ubat melalui API FHIR R4:

- **MedicationKnowledge**: [`/fhir/MedicationKnowledge/DB00609`](/fhir/MedicationKnowledge/DB00609.json)

---

## Sumber Berkaitan

| Sumber | Pautan |
|----------|------|
| DrugBank | [View on DrugBank](https://go.drugbank.com/drugs/DB00609) |
| PubMed | [Search PubMed](https://pubmed.ncbi.nlm.nih.gov/?term=Ethionamide) |
| ClinicalTrials.gov | [Search Trials](https://clinicaltrials.gov/search?intr=Ethionamide) |

---

<div class="disclaimer">
<strong>Penafian</strong><br>
Ramalan penggunaan semula ubat adalah untuk tujuan penyelidikan sahaja. Ramalan ini belum disahkan secara klinikal dan tidak boleh digunakan untuk keputusan perubatan. Sentiasa dapatkan nasihat profesional kesihatan.
</div>
