---
layout: drug
title: "Avelumab"
drugbank_id: "DB11945"
drug_name: "Avelumab"
kg_predictions: 1
dl_predictions: 0
evidence_level: "L4"
indication_count: 10
brands: []
permalink: /drugs/avelumab/
---

# Avelumab

## Gambaran Ringkas

| Atribut | Nilai |
|-----------|-------|
| **Nama Ubat** | Avelumab |
| **ID DrugBank** | [DB11945](https://go.drugbank.com/drugs/DB11945) |
| **Ramalan KG** | 1 |
| **Ramalan DL (≥0.7)** | 0 |
| **Tahap Bukti** | L5 |
| **Status NPRA** | Approved |

---

## Contoh Nama Jenama di Malaysia



---

## Petunjuk Diramalkan

Ubat ini mempunyai **1** ramalan graf pengetahuan dan **0** ramalan pembelajaran mendalam berkeyakinan tinggi.

### Ramalan KG Teratas

| Petunjuk | Sumber |
|------------|--------|
| cutaneous neuroendocrine carcinoma | KG |

---

## Sumber FHIR

Akses data ubat melalui API FHIR R4:

- **MedicationKnowledge**: [`/fhir/MedicationKnowledge/DB11945`](/fhir/MedicationKnowledge/DB11945.json)

---

## Sumber Berkaitan

| Sumber | Pautan |
|----------|------|
| DrugBank | [View on DrugBank](https://go.drugbank.com/drugs/DB11945) |
| PubMed | [Search PubMed](https://pubmed.ncbi.nlm.nih.gov/?term=Avelumab) |
| ClinicalTrials.gov | [Search Trials](https://clinicaltrials.gov/search?intr=Avelumab) |

---

<div class="disclaimer">
<strong>Penafian</strong><br>
Ramalan penggunaan semula ubat adalah untuk tujuan penyelidikan sahaja. Ramalan ini belum disahkan secara klinikal dan tidak boleh digunakan untuk keputusan perubatan. Sentiasa dapatkan nasihat profesional kesihatan.
</div>
