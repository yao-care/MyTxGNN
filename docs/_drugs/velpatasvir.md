---
layout: drug
title: "Velpatasvir"
drugbank_id: "DB11613"
drug_name: "Velpatasvir"
kg_predictions: 4
dl_predictions: 0
evidence_level: "L1"
indication_count: 10
brands: []
permalink: /drugs/velpatasvir/
---

# Velpatasvir

## Gambaran Ringkas

| Atribut | Nilai |
|-----------|-------|
| **Nama Ubat** | Velpatasvir |
| **ID DrugBank** | [DB11613](https://go.drugbank.com/drugs/DB11613) |
| **Ramalan KG** | 4 |
| **Ramalan DL (≥0.7)** | 0 |
| **Tahap Bukti** | L5 |
| **Status NPRA** | Approved |

---

## Contoh Nama Jenama di Malaysia



---

## Petunjuk Diramalkan

Ubat ini mempunyai **4** ramalan graf pengetahuan dan **0** ramalan pembelajaran mendalam berkeyakinan tinggi.

### Ramalan KG Teratas

| Petunjuk | Sumber |
|------------|--------|
| hepatitis C virus infection | KG |
| chronic hepatitis C virus infection | KG |
| hepatitis C virus infection | KG |
| chronic hepatitis C virus infection | KG |

---

## Sumber FHIR

Akses data ubat melalui API FHIR R4:

- **MedicationKnowledge**: [`/fhir/MedicationKnowledge/DB11613`](/fhir/MedicationKnowledge/DB11613.json)

---

## Sumber Berkaitan

| Sumber | Pautan |
|----------|------|
| DrugBank | [View on DrugBank](https://go.drugbank.com/drugs/DB11613) |
| PubMed | [Search PubMed](https://pubmed.ncbi.nlm.nih.gov/?term=Velpatasvir) |
| ClinicalTrials.gov | [Search Trials](https://clinicaltrials.gov/search?intr=Velpatasvir) |

---

<div class="disclaimer">
<strong>Penafian</strong><br>
Ramalan penggunaan semula ubat adalah untuk tujuan penyelidikan sahaja. Ramalan ini belum disahkan secara klinikal dan tidak boleh digunakan untuk keputusan perubatan. Sentiasa dapatkan nasihat profesional kesihatan.
</div>
