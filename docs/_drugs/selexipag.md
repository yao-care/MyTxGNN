---
layout: drug
title: "Selexipag"
drugbank_id: "DB11362"
drug_name: "Selexipag"
kg_predictions: 8
dl_predictions: 0
evidence_level: "L5"
indication_count: 0
brands: []
permalink: /drugs/selexipag/
---

# Selexipag

## Gambaran Ringkas

| Atribut | Nilai |
|-----------|-------|
| **Nama Ubat** | Selexipag |
| **ID DrugBank** | [DB11362](https://go.drugbank.com/drugs/DB11362) |
| **Ramalan KG** | 8 |
| **Ramalan DL (≥0.7)** | 0 |
| **Tahap Bukti** | L5 |
| **Status NPRA** | Approved |

---

## Contoh Nama Jenama di Malaysia



---

## Petunjuk Diramalkan

Ubat ini mempunyai **8** ramalan graf pengetahuan dan **0** ramalan pembelajaran mendalam berkeyakinan tinggi.

### Ramalan KG Teratas

| Petunjuk | Sumber |
|------------|--------|
| pulmonary arterial hypertension | KG |
| pulmonary arterial hypertension | KG |
| pulmonary arterial hypertension | KG |
| pulmonary arterial hypertension | KG |
| pulmonary arterial hypertension | KG |
| pulmonary arterial hypertension | KG |
| pulmonary arterial hypertension | KG |
| pulmonary arterial hypertension | KG |

---

## Sumber FHIR

Akses data ubat melalui API FHIR R4:

- **MedicationKnowledge**: [`/fhir/MedicationKnowledge/DB11362`](/fhir/MedicationKnowledge/DB11362.json)

---

## Sumber Berkaitan

| Sumber | Pautan |
|----------|------|
| DrugBank | [View on DrugBank](https://go.drugbank.com/drugs/DB11362) |
| PubMed | [Search PubMed](https://pubmed.ncbi.nlm.nih.gov/?term=Selexipag) |
| ClinicalTrials.gov | [Search Trials](https://clinicaltrials.gov/search?intr=Selexipag) |

---

<div class="disclaimer">
<strong>Penafian</strong><br>
Ramalan penggunaan semula ubat adalah untuk tujuan penyelidikan sahaja. Ramalan ini belum disahkan secara klinikal dan tidak boleh digunakan untuk keputusan perubatan. Sentiasa dapatkan nasihat profesional kesihatan.
</div>
