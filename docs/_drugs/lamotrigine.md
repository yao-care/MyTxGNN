---
layout: drug
title: "Lamotrigine"
drugbank_id: "DB00555"
drug_name: "Lamotrigine"
kg_predictions: 100
dl_predictions: 99
evidence_level: "L1"
indication_count: 100
brands: []
permalink: /drugs/lamotrigine/
---

# Lamotrigine

## Quick Overview

| Attribute | Value |
|-----------|-------|
| **Drug Name** | Lamotrigine |
| **DrugBank ID** | [DB00555](https://go.drugbank.com/drugs/DB00555) |
| **KG Predictions** | 100 |
| **DL Predictions (≥0.7)** | 99 |
| **Evidence Level** | L1 |
| **NPRA Status** | Approved |

---

## Sample Brand Names in Malaysia



---

## Predicted Indications

This drug has **100** knowledge graph predictions and **99** high-confidence deep learning predictions.

### Top KG Predictions

| Indication | Source |
|------------|--------|
| cutis verticis gyrata | KG |
| epilepsy | KG |
| Lennox-Gastaut syndrome | KG |
| partial epilepsy | KG |
| visual epilepsy | KG |
| cutis verticis gyrata | KG |
| epilepsy | KG |
| Lennox-Gastaut syndrome | KG |
| partial epilepsy | KG |
| visual epilepsy | KG |

---

## FHIR Resources

Access drug data via FHIR R4 API:

- **MedicationKnowledge**: [`/fhir/MedicationKnowledge/DB00555`](/fhir/MedicationKnowledge/DB00555.json)

---

## Related Resources

| Resource | Link |
|----------|------|
| DrugBank | [View on DrugBank](https://go.drugbank.com/drugs/DB00555) |
| PubMed | [Search PubMed](https://pubmed.ncbi.nlm.nih.gov/?term=Lamotrigine) |
| ClinicalTrials.gov | [Search Trials](https://clinicaltrials.gov/search?intr=Lamotrigine) |

---

<div class="disclaimer">
<strong>Disclaimer</strong><br>
Drug repurposing predictions are for research purposes only. These predictions have not been clinically validated and should not be used for medical decisions. Always consult healthcare professionals.
</div>
