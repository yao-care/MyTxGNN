---
layout: drug
title: "Insulin Human"
drugbank_id: "DB00030"
drug_name: "Insulin Human"
kg_predictions: 110
dl_predictions: 20
evidence_level: "L5"
indication_count: 110
brands: []
permalink: /drugs/insulin-human/
---

# Insulin Human

## Quick Overview

| Attribute | Value |
|-----------|-------|
| **Drug Name** | Insulin Human |
| **DrugBank ID** | [DB00030](https://go.drugbank.com/drugs/DB00030) |
| **KG Predictions** | 110 |
| **DL Predictions (≥0.7)** | 20 |
| **Evidence Level** | L5 |
| **NPRA Status** | Approved |

---

## Sample Brand Names in Malaysia



---

## Predicted Indications

This drug has **110** knowledge graph predictions and **20** high-confidence deep learning predictions.

### Top KG Predictions

| Indication | Source |
|------------|--------|
| diabetic ketoacidosis | KG |
| type 2 diabetes mellitus | KG |
| type 1 diabetes mellitus | KG |
| diabetes mellitus (disease) | KG |
| IDDM 1 | KG |
| diabetic ketoacidosis | KG |
| type 2 diabetes mellitus | KG |
| type 1 diabetes mellitus | KG |
| diabetes mellitus (disease) | KG |
| IDDM 1 | KG |

---

## FHIR Resources

Access drug data via FHIR R4 API:

- **MedicationKnowledge**: [`/fhir/MedicationKnowledge/DB00030`](/fhir/MedicationKnowledge/DB00030.json)

---

## Related Resources

| Resource | Link |
|----------|------|
| DrugBank | [View on DrugBank](https://go.drugbank.com/drugs/DB00030) |
| PubMed | [Search PubMed](https://pubmed.ncbi.nlm.nih.gov/?term=Insulin+Human) |
| ClinicalTrials.gov | [Search Trials](https://clinicaltrials.gov/search?intr=Insulin+Human) |

---

<div class="disclaimer">
<strong>Disclaimer</strong><br>
Drug repurposing predictions are for research purposes only. These predictions have not been clinically validated and should not be used for medical decisions. Always consult healthcare professionals.
</div>
