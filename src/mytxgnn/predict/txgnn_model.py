...
def check_dependencies() -> Tuple[bool, List[str]]:
    ...
    # 檢查官方 TxGNN 套件
    # 由於本地 txgnn 模組可能遮蔽官方套件，直接從 site-packages 檢查
    try:
        import sys
        ...
        if site_packages is None:
            ...
            txgnn_path = os.path.join(site_packages, "txgnn")
            init_file = os.path.join(txgnn_path, "__init__.py")

            if not os.path.isdir(txgnn_path) or not os.path.exists(init_file):
                missing.append("TxGNN")
            else:
                # 只檢查檔案是否存在，不實際載入（避免模組名稱衝突）
                # 檢查 TxData.py 和 TxGNN.py 是否存在
                txdata_file = os.path.join(txgnn_path, "TxData.py")
                txgnn_file = os.path.join(txgnn_path, "TxGNN.py")

                if not os.path.exists(txdata_file) or not os.path.exists(txgnn_file):
                    missing.append("TxGNN")
...
# 緩存已載入的官方 TxGNN 模組
_txgnn_official_module = None


def import_official_txgnn():
    """載入官方 TxGNN 套件（避免與本地 txgnn 模組衝突）

    直接從 site-packages 載入官方 TxGNN，避免被本地 txgnn 模組遮蔽。

    Returns:
        官方 TxGNN 模組
    """
    global _txgnn_official_module

    if _txgnn_official_module is not None:
        return _txgnn_official_module

    import sys
    import importlib.util

    # 找到 site-packages 中的官方 TxGNN
    site_packages = _find_site_packages()
    if site_packages is None:
        raise ImportError("找不到 site-packages 目錄")

    txgnn_path = os.path.join(site_packages, "txgnn")
    init_file = os.path.join(txgnn_path, "__init__.py")

    if not os.path.isdir(txgnn_path) or not os.path.exists(init_file):
        raise ImportError(f"找不到官方 TxGNN 套件，請執行: pip install git+https://github.com/mims-harvard/TxGNN.git")

    # 清除可能已經載入的本地模組
    modules_to_remove = [k for k in list(sys.modules.keys()) if k == "txgnn" or k.startswith("txgnn.")]
    for mod in modules_to_remove:
        del sys.modules[mod]

    # 直接從 site-packages 載入
    spec = importlib.util.spec_from_file_location(
        "txgnn", init_file,
        submodule_search_locations=[txgnn_path]
    )
    if spec is None or spec.loader is None:
        raise ImportError("無法載入官方 TxGNN 套件")

    txgnn_pkg = importlib.util.module_from_spec(spec)
    sys.modules["txgnn"] = txgnn_pkg
    spec.loader.exec_module(txgnn_pkg)

    _txgnn_official_module = txgnn_pkg
    return txgnn_pkg
...
class TxGNNPredictor:
    ...
    def setup(self, download_if_missing: bool = True):
        ...
        # 載入官方 TxGNN 套件（避免與本地 txgnn 模組衝突）
        txgnn_official = import_official_txgnn()
        TxData = txgnn_official.TxData
        TxGNN = txgnn_official.TxGNN
    ...
    def _load_node_mappings(self):
        ...
        # 確保 ID 是字串格式
        txgnn_official = import_official_txgnn()
        convert2str = txgnn_official.utils.convert2str
        df = df.copy()
